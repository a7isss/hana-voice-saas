...
- Audio data is Base64 encoded and the only supported format is mulaw with 8000
sample rate.
Call Direction
...
1. Internal Maqsam config (sent by default): this includes data like the numbers and
...
The audio messages will follow the following formats, note that AUDIO represents the Base64
