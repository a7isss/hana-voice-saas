...
export default function AudioSetCreationPage() {
  ...
  const [companyGreetings, setCompanyGreetings] = useState<any[]>([]);
  ...
  const processAndSaveAudioSet = async () => {
    ...
    try {
      ...
      const audioFiles = surveyData.survey.questions.map((question: any, index: number) => ({
      ...
      const audioSetData = {
        ...
        surveyFlow: {
          sequence: audioFiles.map((file: any) => file.id),
          responseHandling: {
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <dl className="space-y-1 text-sm">
                ...
                <div><strong>Files Count:</strong> {(audioSet.metadata as any)?.fileCount || 0}</div>
                <div><strong>Estimated Duration:</strong> {(audioSet.metadata as any)?.estimatedCallTime || 'N/A'}</div>
              </dl>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2 max-h-40 overflow-y-auto">
                {audioSet.audioFiles.slice(0, 5).map((file: any, index: number) => (
