...
export default function HospitalDashboard() {
  ...
  // Auth is now handled by Layout/Guard
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const token = localStorage.getItem('hospital_token');
      if (!token) {
        // Should be handled by Guard, but safe check
        return;
      }
        ...
        <nav className="flex space-x-1 bg-white rounded-lg p-2 shadow-sm border border-gray-200">
          {[
            ...
            <button
              ...
              className={`flex-1 py-4 px-4 text-sm font-medium rounded-lg transition-all text-center ${activeTab === tab.id
                ? `${tab.color} text-gray-900 border-2 shadow-sm`
                : `text-gray-600 hover:text-gray-900 hover:bg-gray-50`
                }`}
              title={tab.description}
            >
              <div className={`text-2xl mb-2 transition-all ${activeTab === tab.id ? 'scale-110' : 'scale-100'
                }`}>
                {tab.icon}
              </div>
                      ...
                      <td className="px-4 py-3 text-sm text-gray-900 text-center">
                        <div className="flex items-center justify-center">
                          {Array.from({ length: 5 }).map((_, i) => (
                            <span
                              key={i}
                              className={`text-lg ${i < Math.floor(campaign.satisfaction_score)
                                ? 'text-yellow-400'
                                : 'text-gray-300'
                                }`}
                            >
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {campaignsData.map((campaign: any) => (
                      <tr key={campaign.id}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${campaign.status === 'completed'
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {patientsData.map((patient: any) => (
                      <tr key={patient.id}>
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${patient.priority_level === 'high'
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900 text-center">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${patient.status === 'active'
                            ? 'bg-green-100 text-green-800'
